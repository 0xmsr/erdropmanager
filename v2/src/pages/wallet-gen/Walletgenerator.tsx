...
function parseTxError(e: any): ParsedTxError {
...
const BytecodeExplorer: React.FC = () => {
  ...
  const analyzeCodeSecurity = async () => {
    ...
    setSecResult(null);
    setSecError('');
    const systemPrompt = `Kamu adalah expert security auditor dan smart contract / code reviewer. Analisis kode yang diberikan secara SANGAT DETAIL untuk menemukan kerentanan keamanan, backdoor, atau kode berbahaya.
...
Berikan skor yang jujur. Jika kode benar-benar aman, katakan aman.`;
    const userPrompt = `Analisis keamanan kode berikut${secLang !== 'auto' ? ` (${secLang})` : ''}:\n\n\`\`\`\n${code}\n\`\`\``;
    try {
      ...
      setSecResult(JSON.parse(cleaned));
                ...
                <div style={{ background:'#070707', border:'1px solid #1e1e1e', borderLeft:'3px solid #f3ba2f', padding:'14px', marginBottom:'14px' }}>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'6px', marginBottom:'10px' }}>
                    {(['slow','standard','fast','manual'] as const).map(mode => {
                      const labels = { slow:'🐢 Slow', standard:'⚡ Standard', fast:'[execute] Fast', manual:'✏️ Manual' };
